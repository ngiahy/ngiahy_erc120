...
async function main() {
  ...
  const replace_contractAddress = "0x62B3178440c1D34d1A663e857F6D93Df91c2bcD4";
  ...
  const replace_functionArgs = [
    "0x62B3178440c1D34d1A663e857F6D93Df91c2bcD4",
    "1000000000000000000",
  ]; // 1 token with 18 decimal places

  const amountTransferred = hre.ethers.formatEther(replace_functionArgs[1]); // Converts to human-readable format (1 token)
  console.log(
    `Transferring ${amountTransferred} tokens to ${replace_functionArgs[0]}...`
  );
  ...
  try {
    ...
    console.log(
      `Transaction completed successfully! ${amountTransferred} tokens transferred to ${replace_functionArgs[0]}.`
    );
    console.log(`Transaction hash: ${transaction.hash}`);
  } catch (error) {
    console.error(
      `Transaction failed! Could not transfer ${amountTransferred} tokens to ${replace_functionArgs[0]}.`
    );
    console.error(
      `Transaction hash: ${
